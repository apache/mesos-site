...
The figure below shows a high-level process flow of the contribution delivery process for the Mesos open source distribution. Details of the process can be obtained from the [submitting-a-patch] (/documentation/latest/submitting-a-patch/) document of the Mesos distribution along with the Getting Started Guidance section of this document.
...
The following information will help provide additional guidance to execute the steps summarized in Contribution Process Overview described above as well as provide helpful hints when following the [submitting-a-patch](/documentation/latest/submitting-a-patch/) document of the Mesos distribution.
...
Detailed steps are provided in the [Mesos documentation](/documentation/latest/getting-started/) to download and build Mesos. When downloading the Mesos distribution there are 2 options to obtain the Mesos source code. For contributing, ensure that the Git option is used.
...
It is important to identify a shepherd before you assign a ticket to yourself. Working with a shepherd will reveal its priority at the current time. To identify a shepherd look at the [maintainers](/documentation/latest/committers/) file to get an idea who to ask to shepherd your JIRA issue.\
...
Guidelines to consider when designing a solution can be found in the [effective-code-reviewing](/documentation/latest/effective-code-reviewing/) document.

## Making Changes

After completing the solution review make the source code changes, build and successfully run relevant test cases. Guidelines for these processes can be found in the [submitting-a-patch](/documentation/latest/submitting-a-patch.md) and [mesos-c++style-guide] (c++-style-guide/).
...
When submitting a patch for review, include all testing done in the submission documentation. Follow the detailed steps found in the [submitting-a-patch](/documentation/latest/submitting-a-patch/) document to submit a patch for review.
